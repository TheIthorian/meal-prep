...
public sealed class IngredientCategoryLlmService
{
    ...
    public IngredientCategoryLlmService(
        IOptions<OpenAIConfiguration> options,
        ILogger<IngredientCategoryLlmService> logger
    ) {
        _logger = logger;
    ...
    private async Task<Dictionary<string, string>?> TryCategorizeChunkAsync(
        ...
        try {
            ClientResult<ChatCompletion> completionResult = await _chatClient!
    ...
    {
        [JsonPropertyName("normalizedIngredientName")]
